...
class Attention(nn.Module):
    ...
    def forward(self, query, key, value, m):
        ...
        # ``-1e9`` overflows under CUDA FP16 autocast.  Use the lowest value
        # representable by the active attention dtype and keep the masked
        # result (``masked_fill`` is not an in-place operation).
